    ...
    public BigDecimal convert(Currency sourceCurrency, Currency targetCurrency, BigDecimal sourceAmount) {
        ...
        try {
            ...
            if (exchangeRate == null) {
                ...
                throw new UnknownExchangeRateException("Missing exchange rate for given currencies");
            }
            return exchangeRate.multiply(sourceAmount).setScale(2, RoundingMode.HALF_EVEN);
        } catch (ExternalServiceFailureException ex) {
            logger.error("Error when looking up for exchange rate: {}", ex.getMessage());
            throw new UnknownExchangeRateException("Error when looking up for exchange rate", ex);
        }
    }
